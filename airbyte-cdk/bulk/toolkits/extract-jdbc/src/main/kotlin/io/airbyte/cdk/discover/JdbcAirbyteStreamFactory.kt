...
interface JdbcAirbyteStreamFactory : AirbyteStreamFactory, MetaFieldDecorator {
    ...
    override fun create(
        ...
        val hasCursorField = hasCursorFields(discoveredStream)

        val syncModes =
            when {
                !isCdc && hasCursorField || isCdc && hasPK ->
                    listOf(SyncMode.FULL_REFRESH, SyncMode.INCREMENTAL)
                else -> listOf(SyncMode.FULL_REFRESH)
            }
        val primaryKey = if (isCdc || hasPK) discoveredStream.primaryKeyColumnIDs else emptyList()
        val stream =
